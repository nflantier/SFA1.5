...
public class BlockLiquefier extends ABlockNBT implements IBlockUsingMaterials{

	public static int renderId;
	
	public BlockLiquefier(Material material) {
		super(material, "Liquefier");
		this.setBlockName("blockLiquefier");
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitx, float hity, float hitz){
    	...
    	if(player.getCurrentEquippedItem()!=null && FluidContainerRegistry.isContainer(player.getCurrentEquippedItem())){
    		if(!world.isRemote){
				...
				if(fluid!=null && fluid.getFluid()==FluidRegistry.WATER && fluid.amount>0){
					TileEntity t = world.getTileEntity(x, y, z);
					if(t instanceof TileLiquefier){
						TileLiquefier tel = (TileLiquefier)t;
						if(tel.tankMelt.getFluidAmount()+fluid.amount<=tel.tankMelt.getCapacity()){
		...
		player.openGui(SFArtifacts.instance, ModGUIs.guiIDLiquefier, world, x, y, z);
		return true;
